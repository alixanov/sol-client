...
const Bakery = () => {
  ...
  const [currentCategory, setCurrentCategory] = useState('Bakery');
  ...
  useEffect(() => {
    const category = shopData.find(
      (cat) =>
        (categoryId && cat.category.toLowerCase() === categoryId.toLowerCase()) ||
        cat.category === 'Bakery'
    );
    if (category) {
  ...
  return (
    ...
    >
      ...
      >
        {products.length > 0 ? (
          products.map((product, index) => (
            ...
            >
              <ProductCard ref={(el) => (productRefs.current[index] = el)}>
                <Link
                  ...
                  >
                    <Typography
                      sx={{
                        ...
                        '&:hover': {
                          color: colors.primaryGradient.split(' ')[1],
                        },
                      }}
                    >
                      {product.name}
                    ...
                    <Box sx={{ mt: 'auto' }}>
                      <PriceTag>${product.usdPrice.toFixed(2)}</PriceTag>
                    </Box>
                  </CardContent>
                </Link>
                <AddToCartButton
          ...
          >
            No products in the {currentCategory} category!
          </Typography>
        )}
      </Grid>
      ...
      >
        <CustomSnackbar ref={snackbarRef} message={snackbarMessage} />
      </Snackbar>
    </Box>
  );
};
